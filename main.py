...
async def handler_status(event):
    ...
    if spam_mode == "automatica":
        ...
        status_parts.append(
            f"🕒 *Avviato automaticamente alle*: {format_time(spam_started_at)}"
            if spam_started_at else "🕒 *Avvio*: N/D"
        )
    else:
        status_parts.append("🧰 *Modalità spam*: Manuale")
        status_parts.append(
            f"🕒 *Inizio spam*: {format_time(spam_started_at)}"
            if spam_started_at else "🕒 *Inizio spam*: N/D"
        )
